// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.22;

import {OFTTest} from "@layerzerolabs/oft-evm-upgradeable/test/OFT.t.sol";
import {MyOFTAdapterUpgradeable} from "../contracts/MyOFTAdapterUpgradeable.sol";
import {EndpointV2Mock} from "@layerzerolabs/test-devtools-evm-foundry/contracts/mocks/EndpointV2Mock.sol";
import {ArtTokenOFT} from "../contracts/ArtTokenUpgradeableOFT.sol";
import {Initializable} from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import {IERC20Errors} from "@openzeppelin/contracts/interfaces/draft-IERC6093.sol";
import {OwnableUpgradeable} from "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import {ERC20CappedUpgradeable} from
    "@openzeppelin/contracts-upgradeable/token/ERC20/extensions/ERC20CappedUpgradeable.sol";

contract ArtTokenTestOFT is OFTTest {
    bytes32 internal constant IMPLEMENTATION_SLOT = 0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc;
    ArtTokenOFT oftUpgradeable;

    address proxy;
    address owner;

    // Test helpers for merkle tree
    bytes32[] public merkleProof;
    bytes32 public merkleRoot;

    uint256 public INITIAL_MINT_AMOUNT;
    uint256 public CLAIM_AMOUNT;

    // Add claimer addresses
    address public claimer1;
    address public claimer2;

    function setUp() public virtual override {
        super.setUp();

        // Define the owner address
        owner = vm.addr(1);

        INITIAL_MINT_AMOUNT = 1_000_000;

        oftUpgradeable = ArtTokenOFT(
            _deployContractAndProxy(
                type(ArtTokenOFT).creationCode,
                abi.encode(address(endpoints[aEid])),
                abi.encodeWithSelector(
                    ArtTokenOFT.initialize.selector,
                    "oftUpgradeable",
                    "oftUpgradeable",
                    owner, // owner is the delegate
                    INITIAL_MINT_AMOUNT
                )
            )
        );

        claimer1 = vm.addr(2);
        claimer2 = vm.addr(3);

        // Emit the owner address for debugging purposes
        emit log_address(owner);

        // Initialize CLAIM_AMOUNT
        CLAIM_AMOUNT = 1000 * 10 ** oftUpgradeable.decimals();

        // Create merkle tree with two addresses
        bytes32[] memory leaves = new bytes32[](2);
        leaves[0] = keccak256(abi.encodePacked(claimer1, CLAIM_AMOUNT));
        leaves[1] = keccak256(abi.encodePacked(claimer2, CLAIM_AMOUNT * 2)); // claimer2 gets double allocation

        // Sort leaves for consistent merkle tree
        if (uint256(leaves[0]) > uint256(leaves[1])) {
            bytes32 temp = leaves[0];
            leaves[0] = leaves[1];
            leaves[1] = temp;
        }

        // Calculate merkle root
        merkleRoot = keccak256(abi.encodePacked(leaves[0], leaves[1]));

        // Generate proof for claimer1
        merkleProof = new bytes32[](1);
        merkleProof[0] = leaves[1]; // If claimer1's leaf is leaves[0]

        // Set merkle root and claimable supply
        vm.prank(owner);
        oftUpgradeable.setMerkleRoot(merkleRoot);

        vm.prank(owner);
        oftUpgradeable.setClaimableSupply(CLAIM_AMOUNT * 3); // Total supply for both claimers
    }

    function test_oft_implementation_initialization_disabled() public {
        bytes32 implementationRaw = vm.load(address(oftUpgradeable), IMPLEMENTATION_SLOT);
        address implementationAddress = address(uint160(uint256(implementationRaw)));

        ArtTokenOFT oftUpgradeableImplementation = ArtTokenOFT(implementationAddress);

        vm.expectRevert(Initializable.InvalidInitialization.selector);
        oftUpgradeableImplementation.initialize("oftUpgradeable", "oftUpgradeable", owner, INITIAL_MINT_AMOUNT);

        EndpointV2Mock endpoint = EndpointV2Mock(address(oftUpgradeable.endpoint()));
        assertEq(endpoint.delegates(address(oftUpgradeable)), owner);
        assertEq(endpoint.delegates(implementationAddress), address(0));
    }

    function test_oft_adapter_implementation_initialization_disabled() public {
        MyOFTAdapterUpgradeable oftAdapter = MyOFTAdapterUpgradeable(
            _deployContractAndProxy(
                type(MyOFTAdapterUpgradeable).creationCode,
                abi.encode(address(cERC20Mock), address(endpoints[cEid])),
                abi.encodeWithSelector(MyOFTAdapterUpgradeable.initialize.selector, owner, INITIAL_MINT_AMOUNT)
            )
        );

        bytes32 implementationRaw = vm.load(address(oftAdapter), IMPLEMENTATION_SLOT);
        address implementationAddress = address(uint160(uint256(implementationRaw)));

        MyOFTAdapterUpgradeable oftAdapterImplementation = MyOFTAdapterUpgradeable(implementationAddress);

        vm.expectRevert(Initializable.InvalidInitialization.selector);
        oftAdapterImplementation.initialize(owner);

        EndpointV2Mock endpoint = EndpointV2Mock(address(oftAdapter.endpoint()));
        assertEq(endpoint.delegates(address(oftAdapter)), owner);
        assertEq(endpoint.delegates(implementationAddress), address(0));
    }

    // Test the basic ERC20 functionality of the MyToken contract
    function testERC20Functionality() public {
        // Impersonate the owner to call mint function
        vm.prank(owner);
        // Mint tokens to address(2) and assert the balance
        oftUpgradeable.mint(address(2), 1000);
        assertEq(oftUpgradeable.balanceOf(address(2)), 1000);
    }

    function testSetStakingContractAddress() public {
        address newStakingContract = address(0x123);

        // Set staking contract address
        vm.prank(owner);
        oftUpgradeable.setStakingContractAddress(newStakingContract);

        // Verify the address was set correctly
        assertEq(oftUpgradeable.stakingContractAddress(), newStakingContract);
    }

    function testSetStakingContractAddress_RevertZeroAddress() public {
        // Attempt to set zero address should revert
        vm.prank(owner);
        vm.expectRevert("Invalid staking contract address");
        oftUpgradeable.setStakingContractAddress(address(0));
    }

    function testSetStakingContractAddress_RevertNonOwner() public {
        address newStakingContract = address(0x123);

        // Attempt to set address from non-owner should revert
        vm.prank(address(2));
        vm.expectRevert(abi.encodeWithSelector(OwnableUpgradeable.OwnableUnauthorizedAccount.selector, address(2)));
        oftUpgradeable.setStakingContractAddress(newStakingContract);
    }

    function testMaxSupply() public {
        uint256 initialSupply = oftUpgradeable.totalSupply();
        uint256 maxSupply = oftUpgradeable.cap();
        uint256 remainingSupply = maxSupply - initialSupply;

        // Impersonate owner
        vm.prank(owner);
        // Mint the remaining supply (should succeed)
        oftUpgradeable.mint(address(2), remainingSupply);

        // Verify total supply equals cap
        assertEq(oftUpgradeable.totalSupply(), maxSupply);

        // Try to mint 1 more token (should fail)
        vm.prank(owner);
        vm.expectRevert(
            abi.encodeWithSelector(ERC20CappedUpgradeable.ERC20ExceededCap.selector, maxSupply + 1, maxSupply)
        );
        oftUpgradeable.mint(address(2), 1);
    }

    function testMintExceedingMaxSupply() public {
        uint256 maxSupply = oftUpgradeable.cap();
        uint256 initialSupply = oftUpgradeable.totalSupply();

        // Impersonate owner
        vm.prank(owner);

        // Try to mint more than cap in one transaction
        vm.expectRevert(
            abi.encodeWithSelector(
                ERC20CappedUpgradeable.ERC20ExceededCap.selector, initialSupply + maxSupply, maxSupply
            )
        );
        oftUpgradeable.mint(address(2), maxSupply);
    }

    function testSupplyWithDecimals() public view {
        // Check decimals
        assertEq(oftUpgradeable.decimals(), 18);

        // Initial supply should be 1,000,000 tokens
        uint256 expectedInitialSupply = 1_000_000 * 10 ** oftUpgradeable.decimals();
        assertEq(oftUpgradeable.totalSupply(), expectedInitialSupply);

        // Max supply should be 1 billion tokens
        uint256 expectedMaxSupply = 1_000_000_000 * 10 ** oftUpgradeable.decimals();
        assertEq(oftUpgradeable.cap(), expectedMaxSupply);

        // Verify remaining supply
        uint256 remainingSupply = oftUpgradeable.cap() - oftUpgradeable.totalSupply();
        assertEq(remainingSupply, 999_000_000 * 10 ** oftUpgradeable.decimals()); // 999 million tokens remaining
    }

    function testGetClaimableSupply() public {
        uint256 expectedSupply = 1000 * 10 ** oftUpgradeable.decimals();

        vm.prank(owner);
        oftUpgradeable.setClaimableSupply(expectedSupply);

        assertEq(oftUpgradeable.getClaimableSupply(), expectedSupply);
    }

    function testSetClaimableSupply() public {
        uint256 newSupply = 1000 * 10 ** oftUpgradeable.decimals();

        // Should revert when non-owner tries to set claimable supply
        vm.prank(address(2));
        vm.expectRevert(abi.encodeWithSelector(OwnableUpgradeable.OwnableUnauthorizedAccount.selector, address(2)));
        oftUpgradeable.setClaimableSupply(newSupply);

        // Should succeed when owner sets claimable supply
        vm.prank(owner);
        oftUpgradeable.setClaimableSupply(newSupply);

        // Verify the new claimable supply
        assertEq(oftUpgradeable.getClaimableSupply(), newSupply);
    }

    function testClaimSuccessClaimer1() public {
        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(claimer2, CLAIM_AMOUNT * 2));

        vm.prank(claimer1);
        oftUpgradeable.claim(CLAIM_AMOUNT, CLAIM_AMOUNT, proof);

        assertEq(oftUpgradeable.balanceOf(claimer1), CLAIM_AMOUNT);
        assertEq(oftUpgradeable.claimedAmount(claimer1), CLAIM_AMOUNT);
        assertEq(oftUpgradeable.getClaimableSupply(), CLAIM_AMOUNT * 2); // Remaining for claimer2
    }

    function testClaimSuccessClaimer2() public {
        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(claimer1, CLAIM_AMOUNT));

        vm.prank(claimer2);
        oftUpgradeable.claim(CLAIM_AMOUNT * 2, CLAIM_AMOUNT * 2, proof);

        assertEq(oftUpgradeable.balanceOf(claimer2), CLAIM_AMOUNT * 2);
        assertEq(oftUpgradeable.claimedAmount(claimer2), CLAIM_AMOUNT * 2);
        assertEq(oftUpgradeable.getClaimableSupply(), CLAIM_AMOUNT); // Remaining for claimer1
    }

    function testPartialClaimClaimer2() public {
        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(claimer1, CLAIM_AMOUNT));

        // First claim - half of allocation
        vm.prank(claimer2);
        oftUpgradeable.claim(CLAIM_AMOUNT * 2, CLAIM_AMOUNT, proof);

        assertEq(oftUpgradeable.balanceOf(claimer2), CLAIM_AMOUNT);
        assertEq(oftUpgradeable.claimedAmount(claimer2), CLAIM_AMOUNT);

        // Second claim - remaining allocation
        vm.prank(claimer2);
        oftUpgradeable.claim(CLAIM_AMOUNT * 2, CLAIM_AMOUNT, proof);

        assertEq(oftUpgradeable.balanceOf(claimer2), CLAIM_AMOUNT * 2);
        assertEq(oftUpgradeable.claimedAmount(claimer2), CLAIM_AMOUNT * 2);
    }

    function testClaimFailuresWithProperMerkle() public {
        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(claimer2, CLAIM_AMOUNT * 2));

        // Try to claim with wrong address
        vm.prank(address(4));
        vm.expectRevert("Invalid merkle proof");
        oftUpgradeable.claim(CLAIM_AMOUNT, CLAIM_AMOUNT, proof);

        // Try to claim more than allocated for claimer1
        vm.prank(claimer1);
        vm.expectRevert("Cannot claim more than allocated");
        oftUpgradeable.claim(CLAIM_AMOUNT, CLAIM_AMOUNT * 2, proof);

        // Successful claim for claimer1
        vm.prank(claimer1);
        oftUpgradeable.claim(CLAIM_AMOUNT, CLAIM_AMOUNT, proof);

        // Try to claim again with claimer1
        vm.prank(claimer1);
        vm.expectRevert("Already claimed full allocation");
        oftUpgradeable.claim(CLAIM_AMOUNT, 1, proof);
    }

    function testClaimWithInsufficientClaimableSupply() public {
        // Set claimable supply to less than claim amount
        vm.prank(owner);
        oftUpgradeable.setClaimableSupply(CLAIM_AMOUNT - 1);

        // Try to claim full amount
        vm.prank(address(2));
        vm.expectRevert("Insufficient claimable supply");
        oftUpgradeable.claim(CLAIM_AMOUNT, CLAIM_AMOUNT, merkleProof);
    }

    function testClaimExceedingRemainingAllocation() public {
        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(claimer2, CLAIM_AMOUNT * 2));

        // First claim succeeds
        vm.prank(claimer1);
        oftUpgradeable.claim(CLAIM_AMOUNT, CLAIM_AMOUNT / 2, proof);

        // Second claim fails because it would exceed total allocation
        vm.prank(claimer1);
        vm.expectRevert("Claim amount exceeds allocation");
        oftUpgradeable.claim(CLAIM_AMOUNT, CLAIM_AMOUNT, proof);
    }

    function testBurnOwnTokens() public {
        // Mint tokens to test address
        vm.prank(owner);
        oftUpgradeable.mint(address(2), 1000);

        // Verify initial balance and supply
        assertEq(oftUpgradeable.balanceOf(address(2)), 1000);
        uint256 initialSupply = oftUpgradeable.totalSupply();
        uint256 initialCap = oftUpgradeable.cap();

        // Burn tokens from address(2)
        vm.prank(address(2));
        oftUpgradeable.burn(500);

        // Verify balance, supply, and cap decreased
        assertEq(oftUpgradeable.balanceOf(address(2)), 500);
        assertEq(oftUpgradeable.totalSupply(), initialSupply - 500);
        assertEq(oftUpgradeable.cap(), initialCap - 500);
        assertEq(oftUpgradeable.totalBurned(), 500);
    }

    function testBurnFromWithAllowance() public {
        // Mint tokens to test address
        vm.prank(owner);
        oftUpgradeable.mint(address(2), 1000);

        // Approve address(3) to spend tokens
        vm.prank(address(2));
        oftUpgradeable.approve(address(3), 750);

        // Verify initial state
        uint256 initialSupply = oftUpgradeable.totalSupply();
        uint256 initialCap = oftUpgradeable.cap();

        // Burn tokens using burnFrom
        vm.prank(address(3));
        oftUpgradeable.burnFrom(address(2), 600);

        // Verify balance, supply, cap, and allowance
        assertEq(oftUpgradeable.balanceOf(address(2)), 400);
        assertEq(oftUpgradeable.totalSupply(), initialSupply - 600);
        assertEq(oftUpgradeable.cap(), initialCap - 600);
        assertEq(oftUpgradeable.allowance(address(2), address(3)), 150);
        assertEq(oftUpgradeable.totalBurned(), 600);
    }

    function testBurnFromWithoutAllowance() public {
        // Mint tokens to test address
        vm.prank(owner);
        oftUpgradeable.mint(address(2), 1000);

        // Try to burn without allowance
        vm.prank(address(3));
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC20Errors.ERC20InsufficientAllowance.selector,
                address(3), // spender
                0, // current allowance
                500 // amount required
            )
        );
        oftUpgradeable.burnFrom(address(2), 500);
    }

    function testBurnMoreThanBalance() public {
        // Mint tokens to test address
        vm.prank(owner);
        oftUpgradeable.mint(address(2), 1000);

        // Try to burn more than balance
        vm.prank(address(2));
        vm.expectRevert(
            abi.encodeWithSelector(
                IERC20Errors.ERC20InsufficientBalance.selector,
                address(2), // sender
                1000, // balance
                1500 // amount required
            )
        );
        oftUpgradeable.burn(1500);
    }

    function testMultipleBurnsUpdateTotalBurned() public {
        // Mint tokens to multiple addresses
        vm.prank(owner);
        oftUpgradeable.mint(address(2), 1000);
        vm.prank(owner);
        oftUpgradeable.mint(address(3), 1000);

        uint256 initialCap = oftUpgradeable.cap();

        // Perform multiple burns
        vm.prank(address(2));
        oftUpgradeable.burn(300);

        vm.prank(address(3));
        oftUpgradeable.burn(400);

        // Verify cumulative burned amount and cap reduction
        assertEq(oftUpgradeable.totalBurned(), 700);
        assertEq(oftUpgradeable.cap(), initialCap - 700);
    }

    function testClaimForSuccess() public {
        // Set up staking contract
        address stakingContract = address(0x123);
        vm.prank(owner);
        oftUpgradeable.setStakingContractAddress(stakingContract);

        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(claimer2, CLAIM_AMOUNT * 2));

        // Claim tokens through staking contract
        vm.prank(stakingContract);
        oftUpgradeable.claimFor(CLAIM_AMOUNT, CLAIM_AMOUNT, proof, claimer1);

        // Verify tokens went to staking contract
        assertEq(oftUpgradeable.balanceOf(stakingContract), CLAIM_AMOUNT);
        // Verify claim is tracked against claimer1
        assertEq(oftUpgradeable.claimedAmount(claimer1), CLAIM_AMOUNT);
        // Verify remaining claimable supply
        assertEq(oftUpgradeable.getClaimableSupply(), CLAIM_AMOUNT * 2);
    }

    function testClaimForFailures() public {
        // First test: Try to claim before setting staking contract
        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(claimer2, CLAIM_AMOUNT * 2));

        vm.prank(address(4));
        vm.expectRevert("Staking contract not set");
        oftUpgradeable.claimFor(CLAIM_AMOUNT, CLAIM_AMOUNT, proof, claimer1);

        // Set up staking contract
        address stakingContract = address(0x123);
        vm.prank(owner);
        oftUpgradeable.setStakingContractAddress(stakingContract);

        // Try to claim from non-staking contract
        vm.prank(address(4));
        vm.expectRevert("Invalid staking contract address");
        oftUpgradeable.claimFor(CLAIM_AMOUNT, CLAIM_AMOUNT, proof, claimer1);
    }

    function testClaimForPartialClaims() public {
        address stakingContract = address(0x123);
        vm.prank(owner);
        oftUpgradeable.setStakingContractAddress(stakingContract);

        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(claimer1, CLAIM_AMOUNT));

        // First partial claim
        vm.prank(stakingContract);
        oftUpgradeable.claimFor(CLAIM_AMOUNT * 2, CLAIM_AMOUNT, proof, claimer2);

        assertEq(oftUpgradeable.balanceOf(stakingContract), CLAIM_AMOUNT);
        assertEq(oftUpgradeable.claimedAmount(claimer2), CLAIM_AMOUNT);

        // Second partial claim
        vm.prank(stakingContract);
        oftUpgradeable.claimFor(CLAIM_AMOUNT * 2, CLAIM_AMOUNT, proof, claimer2);

        assertEq(oftUpgradeable.balanceOf(stakingContract), CLAIM_AMOUNT * 2);
        assertEq(oftUpgradeable.claimedAmount(claimer2), CLAIM_AMOUNT * 2);
    }

    function testClaimForAndDirectClaimInteraction() public {
        address stakingContract = address(0x123);
        vm.prank(owner);
        oftUpgradeable.setStakingContractAddress(stakingContract);

        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(claimer2, CLAIM_AMOUNT * 2));

        // Claim half through staking contract
        vm.prank(stakingContract);
        oftUpgradeable.claimFor(CLAIM_AMOUNT, CLAIM_AMOUNT / 2, proof, claimer1);

        // Try to claim more than remaining through direct claim
        vm.prank(claimer1);
        vm.expectRevert("Claim amount exceeds allocation");
        oftUpgradeable.claim(CLAIM_AMOUNT, CLAIM_AMOUNT, proof);

        // Claim remaining amount through direct claim
        vm.prank(claimer1);
        oftUpgradeable.claim(CLAIM_AMOUNT, CLAIM_AMOUNT / 2, proof);

        // Verify final state
        assertEq(oftUpgradeable.balanceOf(stakingContract), CLAIM_AMOUNT / 2);
        assertEq(oftUpgradeable.balanceOf(claimer1), CLAIM_AMOUNT / 2);
        assertEq(oftUpgradeable.claimedAmount(claimer1), CLAIM_AMOUNT);
    }

    function testClaimForEmitsEvent() public {
        address stakingContract = address(0x123);
        vm.prank(owner);
        oftUpgradeable.setStakingContractAddress(stakingContract);

        bytes32[] memory proof = new bytes32[](1);
        proof[0] = keccak256(abi.encodePacked(claimer2, CLAIM_AMOUNT * 2));

        // Expect the TokensClaimedAndStaked event
        vm.expectEmit(true, true, true, true);
        emit ArtTokenOFT.TokensClaimedAndStaked(claimer1, CLAIM_AMOUNT);

        // Perform claim
        vm.prank(stakingContract);
        oftUpgradeable.claimFor(CLAIM_AMOUNT, CLAIM_AMOUNT, proof, claimer1);
    }
}
