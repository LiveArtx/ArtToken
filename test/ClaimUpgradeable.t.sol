...
contract ArtToken_Claim is ContractUnderTest {
    ...
    function test_should_revert_when_attempting_to_claim_multiple_times_per_day() public {
        ...
        (,uint256 tgeEnd,) = artTokenContractUpgradeable.claimingPeriods();
        (, bytes32[] memory merkleProof) = _claimerDetails();
    ...
    function test_should_revert_when_release_amount_is_greater_than_claimable_supply() public {
     ...
     function test_should_update_user_claim_details_during_TGE_claim() public {
         ...
         assertTrue(artTokenContractUpgradeable.isTGEActive());
        ...
        uint256 dailyReleaseAmount = artTokenContractUpgradeable.calculateDailyRelease(allocatedAmount, expectedTgeReleaseAmount);
        ...
        // User Balance
        assertEq(artTokenContractUpgradeable.balanceOf(claimer1), expectedTgeReleaseAmount);
    }
     ...
     function test_should_update_user_claim_details_after_a_tge_and_vesting_claim() public {
        ...
        (, uint256 tgeEnd, uint256 vestingEnd) = artTokenContractUpgradeable.claimingPeriods();
        vm.warp(tgeEnd + 5 days);
        assert(block.timestamp > tgeEnd && block.timestamp < vestingEnd);


        vm.startPrank(address(claimer1));
        uint256 releaseAmount = artTokenContractUpgradeable.calculateDailyRelease(allocatedAmount, expectedTgeReleaseAmount);

        artTokenContractUpgradeable.claim(allocatedAmount, merkleProof);

        ArtTokenUpgradeable.Claim memory claimDetailsAfterVestingClaim = artTokenContractUpgradeable.claimDetailsByAccount(claimer1);
        ...
        // User Balance
        assertEq(artTokenContractUpgradeable.balanceOf(claimer1), expectedTgeReleaseAmount + releaseAmount);
    }
     ...
     function test_should_update_user_claim_details_during_vesting_claim_only() public {
        ...
        vm.startPrank(address(claimer1));
        uint256 allocatedAmount = CLAIM_AMOUNT;
        uint256 dailyReleaseAmount = artTokenContractUpgradeable.calculateDailyRelease(allocatedAmount, 0);
        ...
        // User Balance
        assertEq(artTokenContractUpgradeable.balanceOf(claimer1), dailyReleaseAmount);
    }
      ...
      function test_should_update_user_balance_after_claiming_everyday_during_vesting_only() public {
        ...
        assertApproxEqAbs(artTokenContractUpgradeable.balanceOf(claimer1), totalClaimed, 100);
    }
    ...
    function test_should_update_user_claim_details_after_vesting_claim_only() public {
        ...
        (, bytes32[] memory merkleProof) = _claimerDetails();
        (,,uint256 vestingEnd) = artTokenContractUpgradeable.claimingPeriods();
        ...
        assertEq(artTokenContractUpgradeable.balanceOf(claimer1), allocatedAmount);
    }
   ...
   function test_should_run_successful_claims_across_all_periods() public {
    ...
    function _executeFullCycleClaim(uint256 _tgeClaimPercentage) internal {
        ...
        ArtTokenUpgradeable.Claim memory afterTGE = artTokenContractUpgradeable.claimDetailsByAccount(claimer1);
        ...
        ArtTokenUpgradeable.Claim memory finalClaim = artTokenContractUpgradeable.claimDetailsByAccount(claimer1);
        ...
        assertEq(artTokenContractUpgradeable.balanceOf(claimer1), allocatedAmount, "Final balance should equal total allocation");
    }
}